    ...
    if (event.key === 'Enter') {
        ...
        if (city) {
            if (!allCities.includes(city)) {
                message.textContent = 'игрок ввел город, которого нет в списке робота'
            } else if (cities.includes(city)) {
                message.textContent = 'игрок ввел город, который уже был'
            } else {
                if (cities.length === 0) {
                    cities.push(city)
                    let robot = randomWord()
                    message.textContent = robot;
                    field.value = ''
                } else {
                    let [first, last] = checkedLetters(city)
                    if (first === last) {
                        cities.push(city)
                        let robot = randomWord()
                        message.textContent = robot
                        ...
                        message.textContent = 'игрок ввел город не на ту букву, на которую заканчивается предыдущий город'
                    }
                }
            }
        }
...
function checkedLetters(city) {
    ...
    let nextuser = cities[cities.length - 1]
    let last = nextuser[nextuser.length - 1].toLowerCase()
    if (last === 'ь' || last === 'ы'  || last === 'й') {
        last = nextuser[nextuser.length - 2].toLowerCase()
    }
    return [first, last]
}
function randomWord() {
    let robot = allCities[Math.floor(Math.random() * (allCities.length - 0 + 1)) + 0]
    let [first, last] = checkedLetters(robot)
    if (!cities.includes(robot) && robot && first === last) {
        cities.push(robot)
        return robot
    } else {
        return randomWord()
